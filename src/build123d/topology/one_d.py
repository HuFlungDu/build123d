    ...
    def _extend_spline(
        ...
        if at_start:
            while extension_factor > 0:
                ...
                raise ValueError("Failed to extend spline")
            ends = (-extension_factor, 1)
            ...
            ends = (0, 1 + extension_factor)
        tangents: list[VectorLike] = [self.tangent_at(p) for p in ends]

        # pnts: list[VectorLike] = [Vector(p) for p in poles]
        pnts2 = [self.position_at(ends[0])]
        if (at_start):
            pnts2.append(self.position_at(0))
        subdivisions = len(poles)*2
        for i in range(1, subdivisions):
            pnts2.append(self.position_at(i/subdivisions))
        if (not at_start):
            pnts2.append(self.position_at(1))
        pnts2.append(self.position_at(ends[1]))
        # pnts: list[VectorLike] = [self.position_at(p) for p in range(ends[0], ends[1], (ends[1]-ends[0])/())]
        # Ignore tangents I guess?
        extended_edge = Edge.make_spline(pnts2)
        assert extended_edge.wrapped is not None

        geom_curve = BRep_Tool.Curve_s(extended_edge.wrapped, extended_edge.param_at(0), extended_edge.param_at(1))
